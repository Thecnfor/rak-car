...
class PID(object):
    ...
    def __call__(self, input_, dt=None):
        ...
        if self._last_error > 0 and error < 0:
            # Reset integral term if we were previously positive and now negative
            self._integral = 0
        elif self._last_error < 0 and error > 0:
            # Reset integral term if we were previously negative and now positive
            self._integral = 0
