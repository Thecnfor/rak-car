...
def spilt_voc_dataset(path_root, val_percent:float=0.2):
    """
    将voc数据集划分为训练集、验证集、测试集
    :param path_root: voc数据集根目录
    :param trainval_percent: 划分训练集和验证集的比例
    :param train_percent: 划分训练集和验证集的比例
    :return:
    """

    # val_percent = 0.2
    # train_percent = 0.8
    # path_root = 'fast_data_det'
    # path_root = 'car_data_coco'
    xml_input_dir = 'annotations'
    xml_dir = os.path.join(path_root, xml_input_dir)
    image_input_dir = 'images'
    image_path = os.path.join(path_root, image_input_dir)
    ...
    # 总体取出给定百分比的个数（测试集与验证集个数）
    num_val = int(num * val_percent)
    #从10%中再取去90%用作验证集的个数
    num_test = int(num_val * val_percent)
    #trainval：包括测试集与验证集
    valid_val = random.sample(xml_list, num_val)#从list中随机获取tv个元素，作为一个片断返回
    test_val = random.sample(valid_val, num_test)

    f_train = open(os.path.join(path_root, 'train.txt'), 'w')  # 以写入的方式打开train.txt，没有则创建
    f_valid = open(os.path.join(path_root, 'valid.txt'), 'w')
    f_test = open(os.path.join(path_root, 'test.txt'), 'w')

    f_write = f_test    # 定义链接变量

    for i in xml_list:
        # 获取对应的xml名字
        xml_name = os.path.splitext(total_xml[i])[0]
        xml_path = os.path.join(xml_dir, total_xml[i])
        # 对应的图片目录
        img_path = os.path.join(image_path, xml_name+'.jpg')
        if os.path.exists(img_path):

            # print(img_path)
            # print(xml_path)
            # 相对地址
            img_path_relative = os.path.join(image_input_dir, xml_name+'.jpg')
            xml_path_relative = os.path.join(xml_input_dir, xml_name+'.xml')
            # ET打开xml文件，并获取name属性
            ...
            f_write.write(img_path_relative)
            f_write.write(' ')
            f_write.write(xml_path_relative)
            f_write.write('\n')
    ...
    # cylinder2
    # cylinder3
    # 保存label_list到txt，类似一行一个类
    print(label_list)
    with open(os.path.join(path_root, 'label_list.txt'), 'w') as f:
...
# python tools/x2coco.py --dataset_type voc --image_input_dir dataset\car_data_voc\images --output_dir dataset\car_data_voc\cocoanotations --voc_anno_dir dataset\car_data_voc --voc_anno_list dataset\car_data_voc --voc_anno_list dataset\car_data_voc\test.txt --voc_label_list dataset\car_data_voc\label_list.txt --voc_out_name test.json