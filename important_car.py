...
class MyCarTask:
    ...
    def front(self):
        ...
        if side == -1:
            ...
            self.my_car.set_pose_offset([0, 0, -math.pi / 35 * side])
            time.sleep(0.3)

            self.my_car.set_pose_offset([0.28, 0, 0])

        else:
            self.my_car.task.arm.set(0.18, 0)
            self.my_car.task.arm.switch_side(-1)
            ...
            self.my_car.lane_dis_offset(0.2, 0.58)
            self.my_car.set_pose_offset([0, 0.035, 0])
            self.my_car.set_pose_offset([0, 0, -math.pi / 12 * side])
            time.sleep(0.3)
            self.my_car.set_pose_offset([0.28, 0, 0])
        ...
        self.my_car.lane_sensor(0.2, value_h=0.3, sides=1, stop=True)
        self.my_car.task.arm.set(0.1, 0.18)
        self.my_car.task.arm.switch_side(1)
        self.my_car.task.arm.set(0.1, 0)
        ...
        # t.start()
        # t.join(timeout=3)
        self.my_car.set_pose_offset([0, 0.085, 0], 1)
        self.my_car.set_pose_offset([0, -0.065, 0], 1)
        ...
        self.my_car.lane_dis_offset(0.3, 4.65)

        # my_car.set_pose(pose, vel=[0.2, -0.2, math.pi/3])

        self.my_car.set_pose_offset([0, 0, math.pi / 35])

        ######## eject1
        self.my_car.task.eject(1)
        self.my_car.set_pose_offset([0.7, 0, 0])
    ...
    def end(self):
        ...
        time.sleep(0.5)
        self.my_car.lane_det_location_v4(0.2, tar, side=-1, dis_out=1)  # 巡航找文字
        try:
            text = self.my_car.get_ocr_list()[0]  # 获取文字
        ...
        self.my_car.lane_det_location_vert(0.135, tar, side=-1, dis_out=1)
        ...
        if y < 0 or y == 0:
            ...
            self.my_car.task.arm.set_arm_angle(90)  # 手臂向外
            time.sleep(0.5)
        ...
        time.sleep(1)
        self.my_car.lane_det_location_v4(0.2, tar, side=1, dis_out=1)  # 巡航找文本
        try:
            text = self.my_car.get_ocr_list()[0]  # 获取文字
            foods = answer.ask1(text)
        ...
        self.my_car.lane_dis_offset(0.3, 1)
        ...
        ########
        self.my_car.lane_dis_offset(0.3, 1)
        self.my_car.set_pose_offset([-0.02, 0, 0], 1)
        ...
        self.my_car.lane_det_location_v4(0.2, tar, side=-1, dis_out=1)  # 巡航到第一列有文字的地方
        try:
        
            text1 = self.my_car.get_ocr_list()  # 获取文字
            print(text1)
        except Exception as e:
            print(e)
        ...
        self.my_car.lane_det_location_v4(0.2, tar, side=-1, dis_out=1)  # 巡航到第二列有文字的地方
        
        try:
        
            text2 = self.my_car.get_ocr_list()  # 获取文字
            print(text2)
        except Exception as e:
            print(e)
        ...
        if answer_dish == 1 or answer_dish == 3:
            ...
            self.my_car.task.arm.set(0.02, 0.132)  # 伸手去放物品
