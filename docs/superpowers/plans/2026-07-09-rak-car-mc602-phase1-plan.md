...
**Goal:** 今晚 2026-07-09 完成 Jetson 端 MC602 通用驱动 + 部署件,明天同事就能在 dev 机器上远程调 `/mc602/*` service 和订阅 `/mc602/state/raw` 并行开发底盘/臂/枪/LLM。
...
- **ROS2 接口命名空间**:`/mc602/*`(service 和 topic 一致),不暴露 SDK 内部 dev_id。
...
class MC602Node(Node):
    ...
    def __init__(self) -> None:
        ...
        self.create_service(Buzzer, '/mc602/buzzer', self._on_buzzer)

        # 话题
        self._raw_pub = self.create_publisher(RawState, '/mc602/state/raw', 10)
        self._heartbeat_pub = self.create_publisher(Header, '/mc602/heartbeat', 10)
...
# 2. 拉数据
ros2 topic echo /mc602/state/raw
ros2 topic echo /mc602/heartbeat

# 3. 调服务
ros2 service list | grep /mc602
ros2 service call /mc602/read_battery vehicle_wbt_smartcar_msgs/srv/ReadBattery
ros2 service call /mc602/buzzer vehicle_wbt_smartcar_msgs/srv/Buzzer "{freq_hz: 440, duration_ms: 200}"
...
### `/mc602/heartbeat` (std_msgs/Header)
...
class ChassisNode(Node):
    def __init__(self):
        ...
        self.create_subscription(RawState, '/mc602/state/raw', self._on_raw, 10)
...
- ❌ 老 C++ `ros2_control` 栈仍在运行:Phase 4 退役,**注意 chassis/arm 服务可能被双向消费**——同事开发时暂时不用老 C++ 的话题(`/cmd/vel_safe` 等),只调 `/mc602/*`
```
...
你的节点会通过 LAN DDS 自动发现 Jetson 端的 `/mc602/*`,**不需要 SSH Jetson**。
...
- 12 service: `/mc602/set_wheels`、`/mc602/read_encoders`、`/mc602/reset_encoders`、
  ...
  `/mc602/read_analog`、`/mc602/buzzer`
- 2 topic: `/mc602/state/raw`(50 Hz RawState.msg)、`/mc602/heartbeat`(1 Hz)
...
- [ ] **Step 3: 验证 `/mc602/state/raw` 数据流**

```bash
source install/setup.bash
ROS_DOMAIN_ID=42 timeout 3 ros2 topic echo /mc602/state/raw
```
