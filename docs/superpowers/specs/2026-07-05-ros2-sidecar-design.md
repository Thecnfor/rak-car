...
| 刷机 | 计划中 — 刷机后必须能用 ROS2 sidecar (`/opt/ros/humble` 或 iron) |
...
|---------|------|:----:|-----|------|
| `/vehicle_wbt/camera/front/image_raw` | `sensor_msgs/Image` | 30 Hz | BEST_EFFORT, depth=2 | 前向 V4L2 摄像头 |
| `/vehicle_wbt/camera/side/image_raw` | `sensor_msgs/Image` | 30 Hz | BEST_EFFORT, depth=2 | 侧向摄像头 (可选) |
| `/vehicle_wbt/imu/data` | `sensor_msgs/Imu` | 100 Hz | BEST_EFFORT | 9-DOF IMU (若装备) |
| `/vehicle_wbt/ir/front` | `std_msgs/Int32MultiArray` | 20 Hz | RELIABLE | 红外测距 (P1-P8) |
...
| Topic 名 | 类型 | 频率 | 用途 |
|---------|------|:----:|------|
| `/vehicle_wbt/perception/lane` | `vehicle_wbt_msgs/LaneResult` | 30 Hz | 车道线分割结果 |
...
| `/vehicle_wbt/joint_states` | `sensor_msgs/JointState` | 50 Hz | 机械臂 6 关节 + 舵机 |
...
3. **ROS2 Humble (LTS)**: 对应 Ubuntu 20.04 + JetPack 5.x
...
| branch-strategy.md | [../contributing/branch-strategy.md](../contributing/branch-strategy.md) | 待写 |
| migration-plan.md | [../migration/ros2-migration.md](../migration/ros2-migration.md) | 待写 |
| hardware-port-mapping.md | [../hardware-port-mapping.md](../hardware-port-mapping.md) | **待补 — URDF 生成 source of truth** |
...
1. `docs/hardware-port-mapping.md` — URDF 生成的 source of truth, 表格需含:
   ...
   - 3 个舵机 (S2, S3, S7)
   - 8 个 IO 引脚 (P1-P8) + 红外 + LED + Beep
   - 2 个摄像头位置 (前/侧)
   - 底盘尺寸 (麦轮直径、轴距、轮距)
2. `docs/contributing/branch-strategy.md` — 4 人并行包开发的分支约定
3. `docs/migration/ros2-migration.md` — phase 1→4 任务分解为可抢 issue
