...
private:
  ...
  // Guards factories_ — registration happens at static-init from multiple
  // translation units, so the registry is genuinely shared state.
...
#define REGISTER_TASK(task_name, class_name)                          \
  ...
  struct task_register_##class_name {                                  \
    ...
    {                                                                  \
      ::hardware::TaskRegistry::instance().register_task( \
        task_name, [](::rclcpp::Node * node) -> std::unique_ptr<::hardware::BaseTask> { \
