...
//
// Subscribes:
//   /rak/safety/heartbeat (std_msgs/Empty)
//   /rak/safety/estop (std_msgs/Bool)
//   /rak/safety/mode_cmd (std_msgs/String)
//   /rak/cmd/vel_raw (geometry_msgs/Twist)
// Publishes (after gating):
//   /rak/cmd/vel_safe (geometry_msgs/Twist)

#include "hardware/safety_gate_logic.hpp"
  ...
  : Node("safety_gate_node", options)
  {
    ...
    estop_sub_ = this->create_subscription<std_msgs::msg::Bool>(
      ...
      [this](const std_msgs::msg::Bool::SharedPtr msg) {
        ...
        if (estop_pressed_) {
          mode_ = vwh::GateMode::E_STOP;
        }
      });

    mode_sub_ = this->create_subscription<std_msgs::msg::String>(
      "/rak/safety/mode_cmd", 10,
      [this](const std_msgs::msg::String::SharedPtr msg) {
        std::lock_guard<std::mutex> lock(state_mutex_);
        if (msg->data == "AUTO") mode_ = vwh::GateMode::AUTO;
        else if (msg->data == "MANUAL") mode_ = vwh::GateMode::MANUAL;
        else if (msg->data == "DEV_TEST") mode_ = vwh::GateMode::DEV_TEST;
        else if (msg->data == "E_STOP") mode_ = vwh::GateMode::E_STOP;
      });
...
private:
  void apply_and_publish(const geometry_msgs::msg::Twist & raw_cmd)
  {
    ...
    const auto decision = vwh::apply_safety_gate(
      in, raw_cmd.linear.x, raw_cmd.linear.y, raw_cmd.angular.z);

    if (static_cast<bool>(decision.reason & vwh::GateDecision::Reason::ESTOP)) {
      RCLCPP_WARN_THROTTLE(
        this->get_logger(), *this->get_clock(), 1000, "E-STOP active");
    }
    if (static_cast<bool>(decision.reason & vwh::GateDecision::Reason::MODE_DROPPED)) {
      RCLCPP_DEBUG_THROTTLE(
        this->get_logger(), *this->get_clock(), 1000,
        "Dropped cmd (AUTO mode)");
    }
    if (static_cast<bool>(decision.reason & vwh::GateDecision::Reason::RATE_LIMITED)) {
  ...
  vwh::GateMode mode_{vwh::GateMode::AUTO};
