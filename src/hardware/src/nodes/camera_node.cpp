...
//
// Publishes 5 streams under /rak/sensors/camera/<id>/:
//
...
//   5. camera_meta       msgs/msg/CameraMeta   1 Hz
//
...
#include <msgs/msg/camera_meta.hpp>
...
public:
  ...
  : Node("camera_node", options)
  {
    ...
    ns_ = "/rak/sensors/camera/" + camera_id_;
    ...
    pub_meta_ = this->create_publisher<msgs::msg::CameraMeta>(
...
private:
  ...
  void open_capture()
  {
    ...
    // framerate bit (0x04). The Aveo SP2812 ships with dynamic framerate
    // ON: in dim light the auto-exposure lengthens the exposure and drags
    // the frame rate down to ~9-10 fps even when 30 fps was requested and
    // the interval was set (v4l2-ctl --set-parm=30 proves the same). With
    // dynamic framerate off, the rate locks near 30 fps and exposure clamps
    // to the frame period instead (slightly darker in very dim light, but
    // the frame cadence is stable — the right trade-off on a robot).
    // Measured on Orin 2026-08-08: 9.2 → 27.2 fps.
    cap_->set(cv::CAP_PROP_AUTO_EXPOSURE, 3.0);
  ...
  rclcpp::Publisher<msgs::msg::CameraMeta>::SharedPtr pub_meta_;
