...
Subscribes to raw image topics and publishes inference results as ROS2 topics:
  Subscribes: /vehicle_wbt/v1/sensors/camera/<id>/image_raw
  Publishes:   /vehicle_wbt/v1/perception/{lane,detections/task,detections/front,ocr}

When ENABLE_INFERENCE is not set or 0, the bridge stays alive but publishes
empty/mock results so downstream consumers (task_runner, nav2) can be tested
without a real GPU inference backend.

Architecture (per ros2-sidecar-design.md §Inference Bridge):
  - The sidecar is an OBSERVER, not a model runner. Real inference runs in
    a separate Python process (InferServer) that the main process manages.
  - This bridge bridges camera images → perception topics via ROS2.
  - Real model integration happens in Phase 6-7 (Python sidecar Phase 2).

Spec: docs/superpowers/specs/2026-07-05-ros2-sidecar-design.md §Inference Bridge
"""
...
from vehicle_wbt_platform_cpp.msg import LaneResult, DetectionArray
...
class InferenceBridgeNode(Node):
    ...
    def __init__(self) -> None:
        ...
        # --- Parameters ---
        self.declare_parameter("lane_topic", "/vehicle_wbt/v1/perception/lane")
        self.declare_parameter("task_detect_topic",
                               "/vehicle_wbt/v1/perception/detections/task")
        self.declare_parameter("front_detect_topic",
                               "/vehicle_wbt/v1/perception/detections/front")
        self.declare_parameter("ocr_topic",
                               "/vehicle_wbt/v1/perception/ocr")
        ...
        mock_rate = self.get_parameter("mock_rate_hz").as_double()
