...
车端（Jetson 无 node）由本模块挂出 —— 与 /v1 API、/video_feed 同源同端口，
前端一律用根路径绝对地址访问。

挂载方式（版本兼容注意）：StaticFiles 必须 `app.mount()` 直接挂在 FastAPI
app 上 —— 老版本 starlette（车端 py3.8 / fastapi 0.110.x）的
`include_router` 不携带 APIRouter 内的 Mount，会导致 /console/ 404。
"""
...
def mount_web_console(app):
    """构建产物存在时把 StaticFiles 直接挂到 app（必须在 include_router 之后调用）。"""
    if not _STATIC_DIR.is_dir():
        return False
    try:
    ...
    app.mount("/console", StaticFiles(directory=str(_STATIC_DIR), html=True),
              name="web_console")
    return True
