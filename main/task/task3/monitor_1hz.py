#!/usr/bin/python3
# -*- coding: utf-8 -*-
r"""main/tasks/task333/monitor_1hz.py - 1Hz pest/beneficial position monitor

Every 1 second:
  1. Read task_feed -> list of animals in view
  2. For each animal with score >= min_score:
       - grab frame, crop bbox, base64 encode
       - call ERNIE VL to judge PEST(0) or BENEFICIAL(1)
       - skip if same position within cooldown
  3. Print summary of which positions have PEST / BENEFICIAL

Press Ctrl+C to stop.

Usage:
    cd "C:\Users\花花世界\Desktop\天道酬勤\rak-car"
    python -m main.tasks.task333.monitor_1hz
    python -m main.tasks.task333.monitor_1hz --interval 1.0 --cooldown 5.0
"""
...
def main():
    ap = argparse.ArgumentParser(description="1Hz pest/beneficial monitor")
    ...
    ap.add_argument("--cooldown", type=float, default=5.0,
                    help="seconds between LLM calls for same position")
    ...
    ap.add_argument("--no-llm", action="store_true",
                    help="disable LLM, just report visual positions")
    ap.add_argument("--streamer", default=None)
    args = ap.parse_args()

    if not args.no_llm:
        if not HAS_ERNIE:
            print("[fatal] llm_ernie not importable", file=sys.stderr)
            sys.exit(2)
    ...
    # verdict cache: pos_key -> {"label": "PEST"/"BENEFICIAL", "analysis": ..., "ts": float}
    ...
    try:
        while True:
            ...
            for det in animals:
                ...
                if args.no_llm or frame is None:
                    frame_results.append({
                        "xc": xc, "yc": yc, "score": score,
                        "label": "?",
                        "reason": "llm disabled" if args.no_llm else "no frame",
                        "source": "n/a",
                    })
                    continue
                ...
                if not crop:
                    frame_results.append({
                        "xc": xc, "yc": yc, "score": score,
                        "label": "?", "reason": "empty crop", "source": "n/a",
                    })
                    continue
            ...
            for r in frame_results:
                parts.append(
                    f"({r['xc']:+.2f},{r['yc']:+.2f})={r['label']} sc={r['score']:.2f}"
                    f"[{r['source']}]"
                )
            print(
                f"[tick {tick:>5}] ts={ts_print:.3f}  "
