#!/usr/bin/python3
# -*- coding: utf-8 -*-
r"""main/tasks/task333/arm_seq_v8.py - sequenced arm motion v8
...
Usage:
    ...
    python -m main.tasks.task333.arm_seq_v8
    python -m main.tasks.task333.arm_seq_v8 --y1 -0.150 --hand-angle -90 --x -0.100
"""
...
def main():
    ap = argparse.ArgumentParser(description="arm sequence v8 "
                                              "(y=-150 → hand=-90 → x=-100)")
    ...
    ap.add_argument("--x", type=float, default=-0.100,
                    help="step 3 x target (m, default -0.100 = -100mm)")
    ap.add_argument("--angle-speed", type=int, default=100,
                    help="set_hand_angle speed (default 100)")
    ...
    print("[seq] starting arm sequence v8", flush=True)
    b = read_arm(client)
    ...
    # 1) y -> -150mm
    print(f"\n[1/3] move_y_position({args.y1}) ...", flush=True)
    ...
    print(f"\n[2/3] set_hand_angle({args.hand_angle}, {args.angle_speed}) ...",
          flush=True)
    ...
    # 3) x -> -100mm
    print(f"\n[3/3] move_x_position({args.x}) ...", flush=True)
