#!/usr/bin/python3
# -*- coding: utf-8 -*-
r"""main/tasks/task333/arm_seq_v5.py - sequenced arm motion v5

动作序列(每步独立,失败抛错停):
  1. arm.move_y_position(-0.150)        -> y = -150mm
  2. arm.set_hand_angle(-60, speed)     -> 手爪舵机角度 -60 度
  3. arm.move_x_position(-0.270)        -> x = -270mm
     + arm.set_arm_angle(90, speed)      -> 机械臂角度 90 度
  4. arm.move_y_position(-0.010)        -> y = -10mm

每步后读 arm_state 打印。

Usage:
    cd "C:\Users\花花世界\Desktop\天道酬勤\rak-car"
    python -m main.tasks.task333.arm_seq_v5
    python -m main.tasks.task333.arm_seq_v5 --hand-angle -60 --arm-angle 90
    python -m main.tasks.task333.arm_seq_v5 --y1 -0.150 --x -0.270 --y2 -0.010
"""
...
def main():
    ap = argparse.ArgumentParser(description="arm sequence v5 "
                                              "(y=-150 → hand=-60 → x=-270+arm=90 → y=-10)")
    ap.add_argument("--y1", type=float, default=-0.150,
                    help="step 1 y target (m, default -0.150 = -150mm)")
    ap.add_argument("--hand-angle", type=float, default=-60.0,
                    help="step 2 hand servo angle (deg, default -60)")
    ...
    ap.add_argument("--y2", type=float, default=-0.010,
                    help="step 4 y target (m, default -0.010 = -10mm)")
    ...
    print("[seq] starting arm sequence v5", flush=True)
    b = read_arm(client)
