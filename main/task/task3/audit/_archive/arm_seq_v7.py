#!/usr/bin/python3
# -*- coding: utf-8 -*-
r"""main/tasks/task333/arm_seq_v7.py - sequenced arm motion v7

动作序列(每步独立,失败抛错停):
  1. arm.move_y_position(-0.150)        -> y = -150mm
  2. arm.set_hand_angle(-80, speed)     -> 手爪舵机角度 -80 度
  3. arm.move_x_position(-0.280)        -> x = -280mm
     + arm.set_arm_angle(90, speed)      -> 机械臂角度 90 度
  4. arm.move_y_position(-0.030)        -> y = -30mm

每步后读 arm_state 打印。

Usage:
    cd "C:\Users\花花世界\Desktop\天道酬勤\rak-car"
    python -m main.tasks.task333.arm_seq_v7
    python -m main.tasks.task333.arm_seq_v7 --hand-angle -80 --x -0.280 --y2 -0.030
"""
...
def main():
    ap = argparse.ArgumentParser(description="arm sequence v7 "
                                              "(y=-150 → hand=-80 → x=-280+arm=90 → y=-30)")
    ap.add_argument("--y1", type=float, default=-0.150,
                    help="step 1 y target (m, default -0.150 = -150mm)")
    ap.add_argument("--hand-angle", type=float, default=-80.0,
                    help="step 2 hand servo angle (deg, default -80)")
    ap.add_argument("--x", type=float, default=-0.280,
                    help="step 3 x target (m, default -0.280 = -280mm)")
    ap.add_argument("--arm-angle", type=float, default=90.0,
                    help="step 3 arm angle (deg, default 90)")
    ap.add_argument("--y2", type=float, default=-0.030,
                    help="step 4 y target (m, default -0.030 = -30mm)")
    ...
    print("[seq] starting arm sequence v7", flush=True)
    b = read_arm(client)
