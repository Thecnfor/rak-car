...
#######################################################################################
    ...
    pose_dict = my_car.lane_det_location_v8_multi(0.2, pts, side=side * -1, dis_out=2)
    print(pose_dict)
    ...
    # t.start()
    # t.join(timeout=3)
    my_car.set_pose_offset([0, 0.091, 0], 2)
    ...
    time.sleep(0.5)
    my_car.lane_det_location_v4(0.2, tar, side=-1, dis_out=3)  # 巡航找文字
    try:
        text = my_car.get_ocr_list()[0]  # 获取文字
    ...
    tar = [index, 0, the_food2, 0.7483181953430176, 0, 0.2, 0.36, 0.7]
    print(f"tar:{tar}")
    ...
    my_car.lane_det_location_v4(0.2, tar, side=-1, dis_out=2)  # 巡航到第一列有文字的地方
    time.sleep(0.5)
    try:
    ...
    time.sleep(0.5)
    my_car.set_pose_offset([-0.02, 0.3, 0], 0.5)
    time.sleep(0.5)
