...
POSE_P_Y_MM: float = -120.0
"""P 姿态 y (mm)。

2026-08-03 现场 (用户): y=-200 时 yellow 在视野里太远看不清,
改为 -120 后 yellow 显著贴近画面。
2026-08-05 用户: -120 → -130 → -140 (持续降低更稳)。
2026-08-05 用户再次拍板: **回到 -120** (视野更广, 后续放球序列 y=-100 也更顺)。
⚠️ 注意: y=-120 显著高于 bin 开仓 y gate 上界 [-205,-145] (BIN_OPEN_Y_MM=-150),
放球段要继续降到 -150 以下出 gate。"""
...
"""P 姿态手爪角度 (°, 2026-08-05 用户拍板 +10°; 演进轨迹 0°→-5°→+5°→+10°, DOWN 偏后 10°)。

⚠️ +10° 超出业务硬限 [-90, +10]° 的旧 docstring (=-90,0]; 实际业务层
safety.py:_HAND_ANGLE_MAX 已在 2026-08-05 放宽到 +10° (P 姿态放宽), 这里
配合更新。详见 ARM_API.md §1.1 set_hand_angle。
调用方如有疑问请查 ARM_API.md §6.3。"""
