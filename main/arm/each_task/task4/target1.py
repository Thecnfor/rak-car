...
动作顺序 (用户 2026-07-22 指定, 2026-07-27 arm 0°→90°, 2026-07-28 y -125→-133):
  ...
  4. move_x(-260)            远距 260mm, split 模式 + realtime 校验 (§7.2.1 / §11)
                                —— 走 common.move_x_with_split (belt-slip 修复后)
                                   (读基准 → 循环 → stall kick → wall 检测 → 总结)
                                ⚠️ **目标超物理墙 (≈ -119.5mm, 见 ARM_API §7.2 & test_x_to_150.py)**
                                   实测会在 -119.5mm 撞墙 → wall_hit 检测 → 立即 break
                                   **最终位置 ≈ -119.5mm, 不到 -260mm**。如要真到位需现场
                                   确认 rail 长度或校准原点。
                                ⚠️ **2026-07-31 用户实测**: belt-slip 已修复, 速度 30→40→80
                                   trust 模式单次不够, 改 split 模式 + 鲁棒参数
                                   (tol=10 / stall=10 / max_stall=5 / wall=-300)
                                   代码保持 -260, split 模式尝试到位
...
⚠️ x 位置走 common.move_x_with_split (2026-07-31 v6++: 用户反馈 belt-slip 已修复,
   trust 模式单次 move_x 不够, 改 split 模式带 stall/wall/overshoot 检测,
   参数放宽适配 belt-slip 修复初期抖动)。
...
def step_target1(client: ArmClient, runner: ArmRunner,
    ...
    print(f"========== {LOG_PREFIX} 完成 "
          f"(y={y_mm} arm={arm_deg}° hand={hand_deg}° x_realtime={x_info.get('actual_x')}) "
          f"==========\n")
    return {
        "ok": True,
        "y_mm": y_mm,
