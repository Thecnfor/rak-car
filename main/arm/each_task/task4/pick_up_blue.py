...
⚠️ x 移动沿革: 旧版走 common.move_x_trust (不 stall kick, 信任 motor PID 内部闭环);
   2026-08 改 composite_run 并行 (y 抬升 ∥ x 横移; 回最终位 ∥ 回抓取位), x 仍是
   绝对位置指令。belt-slip + stall 现场史见 memory/x-axis-belt-slip。
...
def step_pick_up_blue(
    ...
    *,
    ...
    # 4+5. y 抬到中转位 + x 移到蓝 bin (composite_run 并行, 省 ~1s)
