"""task4 / the_final —— **task4 收官脚本** (前置 y→-133 + 开仓 → 完全照搬 target4)。

业务流程 (用户 2026-08-03 指定):
  1. **y 轴升到 -133mm** (runner.move_y, 单纯落点, 不做任何识别 / 不动其他电机)
  2. **开仓** (set_storage_angle 75°) —— 走 ArmClient StorageMixin 接口
     (注意 open_storage 不再读 y / 不做 y gate, 由调用方负责 y 位置)
  3. **完全照搬 target4 流程**: target1 起手 (x_mm=-260) + 1× 初始识别/抓球
     + rounds × (底盘前移 chassis_step_mm + 识别 + 抓球-or-跳过)

⚠️ **2026-08-03 重写**：完全照搬 target4.py 代码, 不调 step_target4。
   重写原因: 用户要求 the_final 自包含, 后续如果 target4.py 改了,
   the_final 不会被自动跟进 (这是有意的双份维护)。

⚠️ **与 target4.py 的代码同步约束**:
  - 改 target4.py 逻辑时, 必须同步改 the_final.py。
  - 函数名同 target4.py: _pick_best_ball / _identify_and_pick / step_target4。
  - 模块独立不互相 import, 函数名不冲突 (虽然同名但作用域在本模块内)。

⚠️ **target1 调用的 x_mm**: 本文件 step_target4 内的 prep 阶段显式传
   x_mm=-260.0 (跟用户在 2026-08-03 之前手工改 target4.py 的版本一致)。

跑法:
    python main/arm/each_task/task4/the_final.py                # 端到端
    python main/arm/each_task/task4/the_final.py --dry-run      # dry 验证
    python main/arm/each_task/task4/the_final.py --rounds 3     # 改循环数
    python main/arm/each_task/task4/the_final.py --no-prep      # 跳过 target1 起手
    python main/arm/each_task/task4/the_final.py --no-open      # 跳过开仓
"""
...
except ImportError:  # pragma: no cover —— 直接 python the_final.py 时无包上下文
...
LOG_PREFIX: str = LOG_PREFIX_TASK4 + "/the_final"

# ---- 前置参数 (the_final 独有) ----
PREP_Y_MM: float = -133.0
"""阶段 1 y 升到的位置 (mm)。2026-07-28 校准的 target1.y, 用户 2026-08-03
指定 the_final 前置开仓前落这个点。"""

OPEN_STORAGE_ANGLE_DEG: int = 75
"""开仓角度 (°)。跟 constants.STORAGE_OPEN_ANGLE_DEG 一致, 这里写一遍
方便看 the_final 入口的所有前置参数。"""

OPEN_STORAGE_SPEED: int = 5
"""开仓速度。"""

OPEN_STORAGE_TIMEOUT_S: float = 10.0
"""开仓 HTTP 同步超时。"""

# ---- 默认参数 (完全照搬 target4.py) ----
...
def _identify_and_pick(
    ...
    *,
    ...
    # 1. 视觉检测 (2026-08-02: 取消 BALL_VERIFIED_* 验证, fetch_balls 不过滤位置)
    try:
...
def step_target4(
    ...
    *,
    ...
    ⚠️ 跟 target4.py 完全相同的实现, 函数名也叫 step_target4 (本模块作用域内
    不冲突)。改 target4.py 逻辑时必须同步改本函数。
    ...
    Args:
        ...
        do_prep: True (默认) 开头跑 target1.step_target1 (显式 x_mm=-260);
                 False 跳过 (假设已在 target1 位姿)。
    ...
    print(f"  准备位姿: {'跑 target1 (x_mm=-260)' if do_prep and not dry_run else '跳过'}")
    ...
    try:
        ...
        if do_prep and not dry_run:
            ...
            prep_res = target1.step_target1(arm_client, runner, x_mm=-260.0)
...
def _step_prep_y(runner: ArmRunner, *, y_mm: float = PREP_Y_MM) -> dict:
    """前置 1/3: y 轴升到 y_mm (单纯落点)。"""
    print(f"\n========== {LOG_PREFIX} [prep 1/2] y 轴升到 {y_mm}mm ==========")
    print(f"  runner.move_y({y_mm}, timeout=30.0)  (绝对位置指令, 出 y 保护区)")
    runner.move_y(y_mm, timeout=30.0)
    print(f"  {LOG_PREFIX} ✅ y 已到位 {y_mm}mm")
    return {"y_mm": y_mm}


def _step_prep_open_storage(
    client: ArmClient,
    *,
    angle_deg: int = OPEN_STORAGE_ANGLE_DEG,
    speed: int = OPEN_STORAGE_SPEED,
    timeout: float = OPEN_STORAGE_TIMEOUT_S,
) -> dict:
    """前置 2/3: 开仓 (set_storage_angle)。

    走 ArmClient.StorageMixin.set_storage_angle (ArmRunner 没暴露此接口)。
    完全照搬 open_storage.step_open_storage 的核心调用, 但因为函数名一致
    会跟兄弟模块冲突, 这里直接调底层。
    """
    print(f"\n========== {LOG_PREFIX} [prep 2/2] 开仓 (angle={angle_deg}°) ==========")
    print(f"  client.set_storage_angle(angle={angle_deg}, speed={speed}, "
          f"timeout={timeout})  (ArmClient.StorageMixin)")
    ...
    print(f"  {LOG_PREFIX} ✅ 开仓完成 (angle={angle_deg}°, speed={speed})")
    return {
        "ok": True,
        "storage": "OPEN",
        "angle_deg": angle_deg,
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="task4 the_final: y→-133 + 开仓 → 完整 target4 流程 (照搬 target4)",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    # ---- the_final 前置参数 ----
    p.add_argument("--prep-y", dest="prep_y", type=float, default=PREP_Y_MM,
                   help=f"前置 y 升到的位置 (mm), 默认 {PREP_Y_MM}")
    p.add_argument("--no-open", dest="no_open", action="store_true",
                   help="跳过开仓 (前置 y 后直接 target4)")
    # ---- 完全照搬 target4 的 CLI ----
...
def main(argv=None) -> int:
    """the_final 入口: 前置 (y + 开仓) + 完全照搬的 step_target4。"""
    t_total = time.monotonic()
    args = build_parser().parse_args(argv)

    # CLI → return_x_mm 转换 (跟 target4.py main 一致)
    if args.no_return:
    ...
    # ---- 前置 1/2: y 升 -133 ----
    try:
        _step_prep_y(runner, y_mm=args.prep_y)
    except Exception as e:
        print(f"  [{LOG_PREFIX}] ❌ 前置 y 升失败: {type(e).__name__}: {str(e)[:120]}")
        return 11

    # ---- 前置 2/2: 开仓 (可选 --no-open 跳过) ----
    if not args.no_open:
        try:
            _step_prep_open_storage(arm)
        except Exception as e:
            print(f"  [{LOG_PREFIX}] ❌ 前置开仓失败: {type(e).__name__}: {str(e)[:120]}")
            return 12
    else:
        print(f"\n========== {LOG_PREFIX} [prep 2/2] --no-open 跳过开仓 ==========")

    # ---- 阶段 3: 完全照搬 target4.py 的 step_target4 ----
    print(f"\n========== {LOG_PREFIX} 进入 target4 照搬流程 ==========")
    result = step_target4(
        arm, http,
        runner=runner,
        rounds=args.rounds,
    ...
    print(f"\n[{LOG_PREFIX}] 最终结果: ok={result['ok']}, "
          f"reason={result['reason']}, "
          f"picks={result['picks']}, skips={result['skips']}, "
          f"pick_failures={result['pick_failures']}, "
          f"target4_elapsed={result['elapsed_s']:.1f}s, "
