"""task6 / position1 —— **任务六位置 1** 的 5 步纯臂序列 (顺序固定, 不动底盘)。

按用户 2026-08-04 指定顺序 (y → arm → x → hand → y):
...
⚠️ **顺序关键** (用户硬指定, 不可调换):
  ...
  - Step 3 x=-18: y=-190 保护区外, move_x_with_split wrapper 放行。
...
⚠️ **与 task6/target1.py 的区别**:
  - target1.py = 3 阶段 (底盘后退 + 4 步臂 + OCR 写 liaobiao1), 顺序固定 y → arm → hand → x,
    终态保持 y=-143, 后续业务 (推杆/扫牌/拍照) 由 tuigan.py 负责。
  - position1.py = 5 步纯臂 (无底盘, 无 OCR), 顺序 **y → arm → x → hand → y**,
    终态 y=-95 (近保护区上界, 适合后续"贴近地面"动作)。
  - 两者场景不同: target1 = 主抓取位姿, position1 = 备选/调整位姿。
...
⚠️ **本文件自包含** (与 task6/{tuigan, wenzishibie}.py + task7/{position*.py} 同款):
   ...
   (见 [[task5-rebuild-2026-07-22]]), 自包含可保证 ``python position1.py``
   直接跑不受影响。

跑法:
    python main/arm/each_task/task6/position1.py
    python -m main.arm.each_task.task6.position1
    python main/arm/each_task/task6/position1.py --y-high -180   # 抬高不到 -190
    python main/arm/each_task/task6/position1.py --arm 75        # 大臂不到 +83°
    python main/arm/each_task/task6/position1.py --x-target -30  # x 不那么远
    python main/arm/each_task/task6/position1.py --y-low -100    # y 下降不到 -95
"""
...
LOG_PREFIX: str = "[task6/position1]"
...
⚠️ 与 task6/target1.py 的 arm=-95° 不同: target1 是推牌姿势, 本脚本是备选位姿。
⚠️ runner.set_arm_angle 默认 speed=80, 大扭矩动作; 用户 CLI 暂不暴露 speed。"""

# ==== Step 3: x 滑到 -18mm (近 0 位, 大余量) ====
POS_X_TARGET_MM: float = -15
"""Step 3: x 滑到 -18mm (近 0 位, 距下界 -320 留 302mm 余量)。
...
⚠️ 终态保持 y=-95 + arm=+83° + x=-18 + hand=0°, 适合"贴近地面"动作。
⚠️ y 从 -190 → -95 是 +95mm 下降, runner.move_y 内部 PID 闭环。"""

# ==== 时序常量 ====
ANGLE_SPEED: int = 80
"""大臂 / 手爪舵机速度, 默认 80。与 task6/{target1, tuigan}.py、task7/*.py 一致。"""
...
def run(client: ArmClient, runner: ArmRunner,
    ...
    与 task6/target1.py 的核心区别:
      - target1 = 4 步 (y → arm → hand → x), 终态 y=-143
      - position1 = 5 步 (y → arm → x → hand → y), 终态 y=-95 (近保护区)
      - position1 **没有底盘移动**, **没有 OCR 入库**, 纯臂
    ...
    Args:
        ...
        x_target_mm: Step 3 x 目标位置 (mm, 默认 -18, 近 0 位)
    ...
    # ==== Step 3: x 滑到 -18 (y=-190 保护区外, split 兜底) ====
    print(f"\n  ── Step 3: x → {x_target_mm:.0f}mm (近 0 位, split 兜底) ──")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        description=(
            "task6 position1: 5 步纯臂序列 "
            "(y=-190 → arm=+83° → x=-18mm → hand=0° → y=-95), 不动底盘, 不做 OCR"
        ),
    ...
    p.add_argument("--x-target", type=float, default=POS_X_TARGET_MM,
                   dest="x_target",
                   help="Step 3 x 目标位置 (mm, 默认 -18, 必须在 [-320, +220])")
