"""task7 / the_final_position4 —— **位置 4 (左中)** 投递编排器 (3 阶段: 后退 → the_final_position5 → 前进)。
...
⚠️ **改版历史**:
  - **v1 (2026-08-06)**: 首次新建 —— 3 阶段编排器, 委托 the_final_position5.run()。
    底盘 Phase 1/3 走 _chassis_move_for, 与 position1.py v5 同款。

跑法:
    python main/arm/each_task/task7/the_final_position4.py
    python -m main.arm.each_task.task7.the_final_position4
    python main/arm/each_task/task7/the_final_position4.py --back 200 --forward 200   # 改底盘距离
    python main/arm/each_task/task7/the_final_position4.py --vel 0.05                 # 更稳
"""
...
LOG_PREFIX: str = "[task7/the_final_position4]"
...
"""底盘最大线速度 (m/s), 与 task7/dipan.py / position1.py v5 默认一致。"""
...
def _chassis_move_for(client: ArmClient, dist_mm: float,
    ...
    与 position1.py v5 的 _chassis_move_for 完全相同 (复制过来避免包依赖)。
...
def run(client: ArmClient, runner: ArmRunner,
    ...
    """3 阶段执行: 后退 → the_final_position5.run() (7 步臂, 含 1 次放气) → 前进。
    ...
    """
    t_overall = time.time()
    print(f"\n========== {LOG_PREFIX} run (后退 → the_final_position5 → 前进) ==========")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        description=(
            "task7 the_final_position4 v1: 3 阶段编排器\n"
            "  Phase 1: 底盘后退 13cm\n"
