"""task7 / the_final_get_position2 —— **位置 2 抓取** 的 4 步 (不碰底盘)。

按用户 2026-08-06 新建, 本文件是 ``get_position2.py v3`` 的 1:1 镜像
(命名风格跟 ``the_final.py`` 编排器一致):

  ========== [task7/the_final_get_position2] run (4 步: composite_run → y_down → suck → y_up) ==========
    [1/4] composite_run: arm=86.0° x=-58.0mm y=-190.0mm hand=10.0°  speed=80 timeout=30s
    [1/4] ✅ 4 轴并发到位 (~2-3s)

    [2/4] runner.move_y(-92.0mm)  y 下降到抓取深度 (从 -190 → -92, 距离 98mm, 保护区外 12mm, hand=+10° UP 不撞保护区)

    [3/4] runner.suck()  吸气 (开始抓取, y=-92mm + hand=+10° + x=-58mm)

    [4/4] runner.move_y(-190.0mm)  y 上升回 init (从 -92 → -190, 距离 98mm, 真空保持, 货物随臂上升)
  ========== 完成 (~3-4s) ==========

逻辑同 ``get_position2.py v3`` —— 4 机联动到位高位 + y 下降到抓取深度 + 吸气 + y 抬回高位。
本文件以 ``the_final_get_position2`` 命名 (跟 ``the_final.py`` / ``the_final_position1-6`` 命名风格统一)。
...
⚠️ **业务硬限** (走前要核对, 见 ARM_API §1.1 / §7 + setters.py):
  ...
  - x=-58 ∈ [-320, +220] mm ✓ (**距下界 262mm 余量, 充裕**)
...
⚠️ **不走 move_x_with_split** (与 task1_seeding / position*.py v5 / target.py 同款):
...
⚠️ **本文件自包含** (与 task7/{position1-3 v5, position5 v4+, target, dipan, get_position2 v3, the_final_get_position1, the_final_position1-6}.py 同款):
  只依赖 ``main.arm.ArmClient`` + ``main.arm.ArmRunner``,
  **不 import** ``main.arm.each_task.common.move_x_with_split`` (本版不用),
  也不 import task7 包内任何模块 (包含 ``get_position2`` 也不引用 —— 本文件是镜像不是 alias)。
  原因: task5 包曾被外部清空过一次 (见 [[task5-rebuild-2026-07-22]]),
  自包含可保证 ``python the_final_get_position2.py`` 直接跑不受影响。

⚠️ **跟 get_position2.py v3 关系** (1:1 镜像, 命名风格同 the_final.py):
  - get_position2.py: 老文件名, v3 是 2026-08-06 现场实测通过的 4 步抓取脚本
  - **the_final_get_position2.py**: 1:1 镜像, 文件名跟 task7/the_final.py 编排器命名风格一致
  - 两个文件**逻辑 + 常量完全相同**, 区别只是文件名 + LOG_PREFIX + 内部 docstring 引用
  - 现场用哪个都行; 如果将来要把位置 2 抓取单独抽出用, 推荐用 ``the_final_get_position2.py``
  - **不要**为了对齐结构去引用 ``from . import get_position2`` —— 违反自包含约定
  - (跟 the_final_position5.py vs position5.py / the_final_position2.py vs position2.py 是同款镜像关系)

⚠️ **跟 the_final_get_position1.py 关系** (唯一区别: x):
  - the_final_get_position1: POS_X_TARGET_MM = 0.0 (中位, 抓位置 1 球)
  - **the_final_get_position2: POS_X_TARGET_MM = -58.0** (左移 58mm, 抓位置 2 球)
  - 其他 3 个 Step + 5 个常量 + ok check 模式 + 自包含约定**完全相同**。
  - 现场实测如发现 x 不到位, 同步改 ``--x-target`` 或编辑本常量。
  - **不要**为了对齐结构去引用 ``from . import the_final_get_position1`` —— 违反自包含约定。

⚠️ **改版历史**:
  - **v1 (2026-08-06)**: 首次新建 —— 1:1 镜像 get_position2.py v3 (4 步抓取脚本, x=-58)。
    跟 the_final_get_position1.py 同款镜像模式, 但 1 是 x=0 (中位), 2 是 x=-58 (左移 58mm)。

跑法:
    python main/arm/each_task/task7/the_final_get_position2.py
    python -m main.arm.each_task.task7.the_final_get_position2
    python main/arm/each_task/task7/the_final_get_position2.py --y-down -100   # 改 Step 2 grab 深度 (-92 → -100)
    python main/arm/each_task/task7/the_final_get_position2.py --y-up -130      # 改 Step 1/4 高位 (-190 → -130)
    python main/arm/each_task/task7/the_final_get_position2.py --x-target -75    # 改 Step 1 x (-58 → -75)
"""
...
# ---------- 终态常量 (用户 2026-08-06 v1 新建, 1:1 镜像 get_position2.py v3, x=-58) ----------

LOG_PREFIX: str = "[task7/the_final_get_position2]"
...
⚠️ 跟 get_position2.py v3 的 POS_Y_UP_MM 完全相同 (本文件是 1:1 镜像)。"""

POS_X_TARGET_MM: float = -58.0
"""Step 1 composite_run 的 x_mm 终态 (-58mm, 位置 2 球心)。

⚠️ 必须 ∈ [-320, +220] 软限位 ✓; 距下界 -320 有 262mm 余量, 充裕。
⚠️ **跟 get_position2.py v3 完全相同**; 跟 the_final_get_position1.py 唯一区别:
   the_final_get_position1 用 0.0 (中位, 位置 1 球), 本文件用 **-58.0** (左移 58mm, 位置 2 球)。
⚠️ 现场如发现 x 不到位 (球没在吸盘正下方), 同步改 ``--x-target`` 或编辑本常量。"""
...
⚠️ 跟 get_position2.py v3 的 POS_ARM_DEG 完全相同。"""
...
⚠️ 故意不暴露给 CLI (避免误改改坏业务硬限边界)。想改请编辑本常量。
⚠️ 跟 get_position2.py v3 的 POS_HAND_DEG 完全相同。"""
...
⚠️ 与 position5 v4+ POS_Y_DOWN_MM=-85 不同 (position5 是 -85 保护区外 5mm 偏紧,
   the_final_get_position2 是 -92 保护区外 12mm 更宽, **留 hand=+10° UP 状态下更安全的余量**)。
⚠️ 用 ``--y-down`` 可现场微调。
⚠️ 跟 get_position2.py v3 的 POS_Y_DOWN_MM 完全相同。"""

# ====== 通用 ======

ANGLE_SPEED: int = 80
"""大臂 / 手爪舵机速度 + xy PID speed, 默认 80。与 task7/position*.py / target.py / position5 v4+ / get_position1 v4 / the_final_get_position1 一致。"""
...
def run(client: ArmClient, runner: ArmRunner) -> dict:
    ...
    终态: x=-58, y=-190, arm=+86°, hand=+10°, **真空保持 ON**
    (需 runner.drop_object() 才会放气)。

    ⚠️ **2026-08-06 v1 新建**: 1:1 镜像 get_position2.py v3, 命名风格统一为
       ``the_final_*.py``。作为 "位置 2 抓取" 的纯臂脚本 (不碰底盘)。
    ...
    Returns:
        {
            ...
            "step1_composite": dict,            # Step 1 composite_run(arm=+86°, x=-58, y=-190, hand=+10°) 原始 job dict
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        description=(
            "task7 the_final_get_position2 v1: 4 步 = composite_run(arm=+86° x=-58 y=-190 hand=+10°)\n"
            "  → move_y(-92) → suck() → move_y(-190)  (1:1 镜像 get_position2.py v3)"
        ),
    ...
    p.add_argument("--x-target", type=float, default=POS_X_TARGET_MM,
                   help="Step 1 composite_run 的 x_mm 终态 (mm, 默认 -58, 位置 2 球心, 必须在 [-320, +220])")
    return p
