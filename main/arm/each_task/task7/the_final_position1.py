"""task7 / the_final_position1 —— **位置 1 (上中)** 投递编排器 (3 阶段: 后退 → the_final_position2 → 前进)。

按用户 2026-08-06 新建 (命名风格同 the_final.py 编排器):

  Phase 1: 底盘后退 130mm (13cm)                                          ← 不变
            ↓
  Phase 2: 调用 the_final_position2.run() (3 步臂, 含 1 次放气 Step 2.2)
            2.0 composite_run(arm=+90°, x=-225, y=-172, hand=-20°)        4 机联动 (直接到投递位)
            2.1 runner.drop_object()                                       🆕 放气 (货物落目标位)
            2.2 runner.move_x(0)                                           x 归零撞墙 calibrate
            ↓
  Phase 3: 底盘前进 130mm (13cm)                                          ← 不变

vs 旧版 the_final_position1.py (如果有):
  - 旧版 3 步臂全 inline, 自包含 (1:1 镜像 position2.py)
  - 新版委托 ``the_final_position2.run()`` 处理 3 步臂, **避免逻辑重复**
  - 命名风格同 ``the_final.py`` 编排器, 跟 ``the_final_position3`` 形成对照 (后者方向相反)

⚠️ **底盘移动 (Phase 1/3) 保持不变** (用户 2026-08-06 明确指示):
  - Phase 1 后退 / Phase 3 前进; 与 the_final_position3.py 的方向**相反** (后者是先退后进)。
...
⚠️ **中间臂序列委托给 the_final_position2** (重要):
  - **不要**在本文件复制 3 步臂代码 —— 违反 DRY, 改 the_final_position2 时容易漏改。
  - **不要**用 ``from .the_final_position2 import ...`` 走包导入 —— task7 不一定
    有 __init__.py。本脚本用 sys.path 注入 task7 目录后**直接模块导入**。
  - the_final_position2.run() 返回 dict 含 ``step1_composite / step2_drop / step3_x_return``
    各步原始 job + final_pose, 编排器原样透传给上游, **不解析/不修改**。

⚠️ **业务硬限 + 顺序约束** (放气位置不能乱) 详见 the_final_position2.py docstring:
  - 3 步臂全部 ARM_API §1.1 / §7 软限位 / 保护区约束, 走前要核对。
  - 关键约束: Step 2.1 drop_object() 必须在 Step 2.0 composite_run 之后、Step 2.2 x 归零之前。
  - y=-172 在保护区 [0, -80] **外** 92mm, composite_run 内部不查 y 保护区 (拍板)。

⚠️ **本文件定位**:
  - **不**算 self-contained 脚本 (与 position1-3 v5.py / position5 v4+ / get_position1 v4 等不同)。
  - 是**编排器** —— 跟 the_final.py 一类, 委托子任务 (the_final_position2)。
  - 命名风格统一: ``the_final_position{N}.py`` = "位置 N 投递全流程" 编排器。
  - the_final_position2.py 是从零照抄 position2.py v5.3 的纯臂 3 步脚本 (单独跑也行);
    **the_final_position1 / 3 必须** 委托 the_final_position2。

⚠️ **改版历史**:
  - **v1 (2026-08-06, 首次覆盖)**: 之前是 3 步纯臂脚本 (1:1 镜像 position2.py v5.3),
    现在改为**编排器模式** —— 底盘 Phase 1/3 走 _chassis_move_for,
    中间 Phase 2 委托 ``the_final_position2.run()`` 处理 3 步臂 (含 1 次放气)。
    底盘方向: 后退 → work → 前进 (跟 the_final_position3 相反)。

跑法:
    python main/arm/each_task/task7/the_final_position1.py
    python -m main.arm.each_task.task7.the_final_position1
    python main/arm/each_task/task7/the_final_position1.py --back 200 --forward 200   # 改底盘距离
    python main/arm/each_task/task7/the_final_position1.py --vel 0.05                 # 更稳
"""
...
ARM_PHASE_TIMEOUT_S: float = 60.0
"""Phase 2 the_final_position2.run() 同步超时 (秒)。
3 步臂 + 1 次放气, 现场实测 ~3-4s (composite_run ~2-3s + drop_object <1s + move_x 225mm ~1-2s),
给 60s 兜底。
注意: 这是**编排器等待 Phase 2 返回**的总超时, 不是 SDK 单步超时 (SDK 单步用各自 timeout)。
当前 the_final_position2.run() 不接受 timeout 参数, 此值**暂时用不到**, 保留参数为将来扩展。"""
...
def _chassis_move_for(client: ArmClient, dist_mm: float,
    ...
    与 the_final_position3.py / position1.py v5 的 _chassis_move_for 完全相同
    (复制过来避免包依赖, 跟 the_final_position3 / 4 / 6 完全同款)。
...
def run(client: ArmClient, runner: ArmRunner,
    ...
    """3 阶段执行: 后退 → the_final_position2.run() (3 步臂, 含 1 次放气) → 前进。

    ⚠️ **2026-08-06 v1 覆盖新建**: 编排器模式 —— 底盘 Phase 1/3 走 move_for,
       中间 Phase 2 委托 ``the_final_position2.run()`` 处理 3 步臂 (含 1 次放气)。

    ⚠️ **底盘方向跟 the_final_position3 相反**:
       - **the_final_position1: 后退 → work → 前进**
       - the_final_position3: 前进 → work → 后退

    Args:
        client: ArmClient (底盘 move_for + the_final_position2 内部 composite_run 都用它)
        runner: ArmRunner (委托给 the_final_position2.run(), 本编排器不用)
        ...
        arm_timeout: Phase 2 the_final_position2.run() 同步超时 (秒, 默认 60; 现场实测 ~3-4s)
                     当前 the_final_position2.run() 不接受 timeout 参数, 暂时用不到。

    Returns:
        {
            "ok":              True / False,
            "back_mm":         float,            # Phase 1 距离 (正值, 用户语义)
            "forward_mm":      float,            # Phase 3 距离 (正值, 用户语义)
            "back_job":        dict,             # Phase 1 move_for 原始 job dict
            "forward_job":     dict,             # Phase 3 move_for 原始 job dict
            "phase2_result":   dict,             # 🆕 Phase 2 the_final_position2.run() 原始返回 dict
                                                #    含 step1_composite / step2_drop / step3_x_return + final_pose
            "final_pose": {                       # 🆕 终态 (Phase 2 完成后, 等于 the_final_position2 终态)
                "x_mm":    0.0,
                "y_mm":    -172.0,
                "arm_deg": 90.0,
                "hand_deg": -20.0,
    ...
    """
    t_overall = time.time()
    print(f"\n========== {LOG_PREFIX} run (后退 → the_final_position2 → 前进) ==========")
    print(f"  Phase 1: 后退 {back_mm:.0f}mm")
    print(f"  Phase 2: 调用 the_final_position2.run() (3 步臂, 含 1 次放气, 总超时 {arm_timeout:.0f}s)")
    ...
    # ==== Phase 2: 调用 the_final_position2.run() (3 步臂) ====
    # 委托子任务, 本编排器**不复制** 3 步臂代码。
    # the_final_position2.run() 内部走 SDK 同步, 异常就抛 (本编排器不静默吞)。
    print(f"\n  ── Phase 2: 调用 the_final_position2.run() (3 步臂, 含 1 次放气) ──")
    t_arm = time.time()
    phase2_result = the_final_position2_run(
        client, runner,
        # 注意: the_final_position2.run() 不接受 timeout 参数,
        # 它内部已经用各自的 COMPOSITE_TIMEOUT_S / X_MOVE_TIMEOUT_S。
    ...
    if not isinstance(phase2_result, dict) or not phase2_result.get("ok", False):
        print(f"  ── Phase 2 ❌ the_final_position2.run() 失败: {phase2_result}")
        return {
            "ok": False,
            "failed_step": "phase2_the_final_position2",
            "back_job": back_job,
            "forward_job": None,
            "phase2_result": phase2_result,
            "final_pose": None,
        }
    print(f"  ── Phase 2 ✅ the_final_position2.run() 完成 ({arm_dt:.2f}s) ──")
    ...
    print(f"  终态 (the_final_position2): y=-172mm (保护区 [0, -80] 外 92mm) "
          f"x=0mm (撞墙 calibrate) "
          f"arm=+90° hand=-20°")
    print(f"     注意: hand=-20° (非 UP) 但 y=-172 在保护区外, 后续 set_*_angle 安全。\n")
    ...
    return {
        ...
        "phase2_result": phase2_result,            # 🆕 透传 the_final_position2.run() 返回 dict
        "final_pose": phase2_result.get("final_pose"),  # 🆕 终态从 the_final_position2 取
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        description=(
            "task7 the_final_position1 v1: 3 阶段编排器\n"
            "  Phase 1: 底盘后退 13cm\n"
            "  Phase 2: the_final_position2.run()  (3 步臂, 含 1 次放气)\n"
            "  Phase 3: 底盘前进 13cm"
        ),
