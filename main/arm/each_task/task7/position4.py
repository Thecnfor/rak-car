"""task7 / position4 —— **位置 4** 的位姿序列 (3 阶段: 后退 → 10 步动作 (含 1 次放气) → 前进)。
...
跑法:
    python main/arm/each_task/task7/position4.py
    python -m main.arm.each_task.task7.position4
    python main/arm/each_task/task7/position4.py --back 200 --forward 200   # 改底盘距离
    python main/arm/each_task/task7/position4.py --x-final -240              # 改臂最终位
"""
...
LOG_PREFIX: str = "[task7/position4]"
...
def run(client: ArmClient, runner: ArmRunner,
    ...
    """3 阶段执行: 后退 → 10 步动作 (含 1 次放气 Step 2.06) → 前进 (与 position5.py v3 一致)。
    ...
    """
    t0 = time.time()
    print(f"\n========== {LOG_PREFIX} run (back → 10 步动作 (含 1 次放气) → forward) ==========")
...
def build_parser() -> argparse.ArgumentParser:
    """CLI 参数: 底盘 `--back` / `--forward` + 臂 7 个常量 override (与 position5 v3 同款)。

    `--back` / `--forward` 接收正 mm (用户语义), 内部自动转符号:
      - --back     → move_for x_m = -|back|/1000     (后退)  ← Phase 1
      - --forward  → move_for x_m = +|forward|/1000  (前进)  ← Phase 3
    ...
    p = argparse.ArgumentParser(
        description=(
            "task7 position4: 后退 13cm → 10 步动作 (含 1 次放气, 与 position5 v3 同款) → 前进 13cm"
        ),
