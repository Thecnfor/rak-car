"""task7 / the_final_position3 —— **位置 3 (上中)** 投递编排器 (3 阶段: 前进 → the_final_position2 → 后退)。
...
vs the_final_position1.py (方向相反):
  - the_final_position1: 后退 → work → 前进  (适合"位置 1", 车先远离 1 号位置再回)
  - **the_final_position3: 前进 → work → 后退**  (适合"位置 3", 车先靠近 3 号位置再回)
  - 其他 3 步臂 + 业务硬限 + 顺序约束**完全相同** (委托 the_final_position2)。
  - **不要**为了对齐结构去复制 the_final_position1.py 的代码 —— 同款代码用 import 复用。

⚠️ **底盘移动 (Phase 1/3) 保持不变** (用户 2026-08-06 明确指示):
  - Phase 1 前进 / Phase 3 后退; 与 the_final_position1.py 的方向**相反** (后者是先退后进)。
...
⚠️ **改版历史**:
  - **v1 (2026-08-06, 首次新建)**: 3 阶段编排器, 委托 the_final_position2.run()。
    底盘 Phase 1/3 走 _chassis_move_for, 与 the_final_position1.py 方向**相反**。

跑法:
    python main/arm/each_task/task7/the_final_position3.py
    python -m main.arm.each_task.task7.the_final_position3
    python main/arm/each_task/task7/the_final_position3.py --forward 200 --back 200   # 改底盘距离
    python main/arm/each_task/task7/the_final_position3.py --vel 0.05                 # 更稳
"""
...
LOG_PREFIX: str = "[task7/the_final_position3]"
...
def _chassis_move_for(client: ArmClient, dist_mm: float,
    ...
    与 the_final_position1.py / position1.py v5 的 _chassis_move_for 完全相同
    (复制过来避免包依赖, 跟 the_final_position1 / 4 / 6 完全同款)。
...
def run(client: ArmClient, runner: ArmRunner,
    ...
    """3 阶段执行: 前进 → the_final_position2.run() (3 步臂, 含 1 次放气) → 后退。

    ⚠️ **2026-08-06 v1 新建**: 编排器模式 —— 底盘 Phase 1/3 走 move_for,
       中间 Phase 2 委托 ``the_final_position2.run()`` 处理 3 步臂 (含 1 次放气)。

    ⚠️ **底盘方向跟 the_final_position1 相反**:
    ...
    """
    t_overall = time.time()
    print(f"\n========== {LOG_PREFIX} run (前进 → the_final_position2 → 后退) ==========")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        description=(
            "task7 the_final_position3 v1: 3 阶段编排器\n"
            "  Phase 1: 底盘前进 13cm\n"
