"""task7 / position3 —— **位置 3** 的位姿序列 (3 阶段: 前进 → 7 步臂 (含 1 次放气) → 后退)。
...
按用户 2026-08-04 v4 指定顺序 (与 position2.py v4 实际运行参数和顺序一致):
  ...
  Phase 2: 7 步动作 (与 position2.py v4 完全同款, 含 1 次放气 Step 2.5, 用户"自己写"指示内联不 import)
            ...
            2.4 move_x_with_split(-225mm)        x 滑到位置 3 (-225mm, 跟 position2 同款, push 货物到位)
...
⚠️ **底盘方向与 position1.py 相反**:
  - position1: 先 **back** → arm → **forward**
  - position3: 先 **forward** → arm → **back**
  - CLI 仍然接收 `--forward` / `--back` 两个 mm 距离, 语义不变 (forward=前进,
    back=后退), 只是 phase 顺序相反:
      - position1: Phase 1=back, Phase 3=forward
      - position3: Phase 1=forward, Phase 3=back

⚠️ **底盘移动 (Phase 1/3)**:
  - 走车端 ``car.move_for`` (相对位姿位移, [x_m, 0, 0])。
  - 正值 = 车头方向前进, 负值 = 车头方向后退 (与 task7/dipan.py / position1 同款)。
...
跑法:
    ...
    python -m main.arm.each_task.task7.position3
    python main/arm/each_task/task7/position3.py --forward 200 --back 200   # 改底盘距离
    python main/arm/each_task/task7/position3.py --vel 0.05                 # 更稳
"""
...
LOG_PREFIX: str = "[task7/position3]"

# ==== Phase 1 / 3: 底盘移动 (跟前 version 一致, 保持不变) ====
# 注意: position3 是先前进 → 再后退, phase 顺序与 position1.py 相反。
...
"""底盘最大线速度 (m/s), 与 task7/dipan.py / position1.py 默认一致。"""
...
"""Phase 2.5: x 滑到位置 3 (-225mm, 与 position2.py 实际运行值一致, 2026-08-04 改)。
...
def run(client: ArmClient, runner: ArmRunner,
    ...
    """3 阶段执行: 前进 → 7 步臂 (含 1 次放气 Step 2.5) → 后退 (与 position1.py v4 顺序相反)。
    ...
    """
    t0 = time.time()
    print(f"\n========== {LOG_PREFIX} run (forward → 7 步臂 (含 1 次放气) → backward) ==========")
    ...
    # 2.4 x 滑到位置 3 (-225mm, 跟 position2 同款, push 货物到位)
    print(f"  [2.4] move_x_with_split({POS_X_TO_MM}mm)  x → 位置 3 (split 兜底)")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    与 position1.py 的区别仅 phase 顺序:
      - position1: Phase 1=back, Phase 3=forward
      - position3: Phase 1=forward, Phase 3=back
    """
    p = argparse.ArgumentParser(
        description=(
            "task7 position3: 前进 13cm → 7 步臂 (含 1 次放气, 与 position2 v4 同款) → 后退 13cm"
        ),
