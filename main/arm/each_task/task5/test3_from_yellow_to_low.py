"""task5 / test3_from_yellow_to_low —— 整合冒烟脚本 (黄→低位仓)。
...
流程 (5 阶段, **吸气包住 low_tower + 前面 5s 独立保持 + 末尾归零**):
  阶段 1: get_yellow.py 取黄位姿 (5 步: y=-130 → move_x(-68) → arm=85° → hand=0°(底层直调) → y=-70)
...
⚠️ **关键时序演进 (跟 test1 v5 同步, 2026-07-23 第五次修正)**:
  - v1 (错误): get_yellow → grasp(60s 吸气+放气) → low_tower
  - v2 (错误): get_yellow → grasp(True) → low_tower → sleep(60s) → grasp(False)
  - v3 (逻辑对):  get_yellow → grasp(True) → low_tower → grasp(False)  # 立刻放气
  - v4 (标注):  显式吸气窗口 = [get_yellow 完成, low_tower 完成]
...
⚠️ **跟 test1 差异**:
  ...
  - 取物 (get_yellow) 跟 test1 一致

⚠️ **吸气期间吸盘位姿**: sleep(5s) 时吸盘在 (y=-70, x=-68, hand=DOWN), 离地高度待实测 (y 已改 -70),
   没贴球; 若要真吸球, 在 grasp(True) 之前先 move_y(球位, 比如 -15)。

⚠️ **不动原文件**: get_yellow.py / low_tower.py 一字未改, 只 import 它们的 run() 函数。

跑法:
    python main/arm/each_task/task5/test3_from_yellow_to_low.py            # 默认 hold 5s
    python main/arm/each_task/task5/test3_from_yellow_to_low.py --hold 10  # 改保持秒
    python main/arm/each_task/task5/test3_from_yellow_to_low.py --hold 0   # 跳过保持 (退化为 v3)
"""
...
def run(client: ArmClient, runner: ArmRunner,
    ...
    """整合 4 阶段: get_yellow → grasp(True) → sleep(hold_s) → low_tower → grasp(False)

    ⚠️ 吸气逻辑 (跟 test1 同款 v5):
      - 起点: get_yellow 完成 → move_y(-70) 后 grasp(True) 吸气
      - 独立保持: sleep(hold_s) 秒 — 期间吸盘在 (y=-70, x=-68, hand=DOWN) 持续吸气
    ...
    """
    print(f"\n========== {LOG_PREFIX} run ==========")
    print(f"  吸气窗口: get_yellow 完成 (y=-70) → sleep({hold_s:.1f}s) → low_tower 完成 (x=-169)")
    print(f"  流程: get_yellow → grasp(True) → sleep({hold_s:.1f}s) → low_tower(吸) → grasp(False)")
    ...
    return {
        "ok": True,
        "phases": ["get_yellow", "grasp+sleep", "low_tower", "release", "reset_x"],
        "grasp_window": f"get_yellow_complete(y=-70) → sleep({hold_s:.1f}s) → low_tower_complete(x=-169)",
        "hold_s": hold_s,
        "x_reset": x_reset,
    }
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="task5 test3: get_yellow -> grasp+hold+sleep -> low_tower -> release (黄→低位仓, 吸气包住 low_tower, 5s 独立保持)",
