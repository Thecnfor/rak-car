"""task5 / test4_from_blue_to_low —— 蓝球 → 低位仓 (薄 wrapper)。
...
跑法:
    python main/arm/each_task/task5/test4_from_blue_to_low.py            # 开环 (旧行为)
    python main/arm/each_task/task5/test4_from_blue_to_low.py --hold 10  # 改保持秒
    python main/arm/each_task/task5/test4_from_blue_to_low.py --vision   # 视觉闭环取球
"""
...
def run(client: ArmClient, runner: ArmRunner,
    ...
    """蓝球 → 低位仓。详见 pick_and_place.run_pick_and_place。"""
...
def build_parser() -> argparse.ArgumentParser:
    return build_pick_place_parser(
        "task5 test4: get_blue -> 取球(开环盲吸/--vision 视觉闭环) -> low_tower -> release"
    )
