"""task5 / new_get_blue —— 「取蓝」位姿 + 吸气 + 下探, **仿 main/task/task1_seeding.py 的四机联动**。

业务要求 (2026-08-07 用户):
  1. 仿 ``main/task/task1_seeding.py::_switch_to_place_pose`` 的 4 机联动
     composite_run 模式, 把臂一次性摆到 "取蓝位姿":
         arm = +85°
         x   = 0 mm   (中位)
         y   = -135 mm
         hand = +10°
...
跑法:
    python main/arm/each_task/task5/new_get_blue.py
    python -m main.arm.each_task.task5.new_get_blue
"""
...
LOG_PREFIX: str = "[task5/new_get_blue]"

GET_BLUE_X_MM: float = 0.0
"""取蓝 x (中位)。"""

GET_BLUE_Y_MM: float = -135.0
"""取蓝 y (吸前高距位)。出 y 保护区 [0, -80] 55mm, 远出保护区。"""

GET_BLUE_ARM_DEG: float = 85.0
"""大臂角度 (°)。业务硬限 [+150, -150]° 内 (+85)。
用户 2026-08-07 拍板 (86° → 84° → 85° → 86° → 85°, 最终定 85°, 与 task5/get_blue.py 和 new_get_yellow.py 一致)。

沿革: 2026-08-07 当天调试 86° → 84° → 85° → 86° → 85°, 现场拍板 85°。
与 new_get_yellow.py 取齐。"""

GET_BLUE_HAND_DEG: float = 10.0
"""手爪角度 (°, DOWN 偏后 +10°)。
业务硬限 [-90, +10]° 上界 (2026-08-05 P 姿态放宽后), 与 POSE_P_HAND_DEG 同值。
非 init 姿态 (-90 是 init), composite_run 内部不查保护区, 安全。"""

GET_BLUE_GRASP_Y_MM: float = -66.0
"""吸气后下降到 y=-66 mm (保护区 [0, -80] 内, 只做 move_y, 不改 hand/arm,
不会被 _check_safe 拒)。

沿革: 2026-08-07 用户从 -25 改为 -74 (下探更深), 2026-08-08 用户改为 -66
(下探变浅, 离吸球位远一点)。
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "task5 new_get_blue v1: 4 机联动 composite_run (arm=+85° x=0 y=-135 hand=+10°)\n"
            "  仿 main/task/task1_seeding.py 的 4 机联动模式\n"
            "  → runner.grasp(True) 吸气 → runner.move_y(-66) 下探"
        ),
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--x", type=float, default=GET_BLUE_X_MM, help="composite_run 目标 x (mm)")
    p.add_argument("--y", type=float, default=GET_BLUE_Y_MM, help="composite_run 目标 y (mm)")
    p.add_argument("--arm", type=float, default=GET_BLUE_ARM_DEG, help="composite_run 目标大臂角度 (°)")
    p.add_argument("--hand", type=float, default=GET_BLUE_HAND_DEG, help="composite_run 目标手爪角度 (°)")
    p.add_argument("--grasp-y", type=float, default=GET_BLUE_GRASP_Y_MM,
