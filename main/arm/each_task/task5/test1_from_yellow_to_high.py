"""task5 / test1_from_yellow_to_high —— 整合冒烟脚本 (用户 2026-07-23 第五次调整吸气逻辑)。

吸气逻辑 (用户 2026-07-23 第五次明确):
  - 起点: get_yellow 完成 → move_y(-70) 后立即 grasp(True) 吸气
  - **新增 (v5)**: 吸气保持 5s (sleep 5s, 用户指定), 默认可用 --hold 改
  - high_tower 期间持续吸气 (4 步, 期间吸盘一直吸住)
  - 终点: high_tower 完成 → move_x(-160) 后 grasp(False) 放气

流程 (5 阶段, **吸气包住 high_tower + 前面 5s 独立保持 + 末尾归零**):
  阶段 1: get_yellow.py 取黄位姿 (5 步: y=-130 → move_x(-68) → arm=85° → hand=0° → y=-70)
  阶段 2: grasp(True) 吸气 + sleep(5s) 独立保持 (吸盘在 y=-70 高位, 吸空气; 若想吸球
          需在 get_yellow 完成后加一个 move_y 下探贴球动作, 当前流程未做)
...
⚠️ **关键时序演进 (2026-07-23 第五次修正)**:
  - v1 (错误): get_yellow → grasp(60s 吸气+放气) → high_tower
  - v2 (错误): get_yellow → grasp(True) → high_tower → sleep(60s) → grasp(False)
  - v3 (逻辑对):  get_yellow → grasp(True) → high_tower → grasp(False)  # 立刻放气
  - v4 (标注):  显式吸气窗口 = [get_yellow 完成, high_tower 完成]
...
⚠️ **不动原文件**: high_tower.py / get_yellow.py / grasp_5.py 一字未改, 只 import 它们
   的 run() 函数 (各 run 内部都用 if __name__ == '__main__' 守卫, import 时不会跑 main)。
⚠️ **复用策略**: get_yellow_run / high_tower_run 用别名 import, 避免和新文件 run() 重名。
⚠️ **y 抬回位 (2026-07-23 同步)**: get_yellow.py 默认值 -88 → -70 → -70 (2026-07-29), 本脚本 import
   复用自动跟上, 无需在文件内另写常量。
⚠️ **吸气期间吸盘位姿**: sleep(5s) 时吸盘在 (y=-70, x=-68, hand=DOWN), 离地高度待实测 (y 已改 -70),
   没贴球; 若要真吸球, 在 grasp(True) 之前先 move_y(球位, 比如 -15)。

跑法:
    python main/arm/each_task/task5/test1_from_yellow_to_high.py            # 默认 hold 5s
    python main/arm/each_task/task5/test1_from_yellow_to_high.py --hold 10  # 改保持秒
    python main/arm/each_task/task5/test1_from_yellow_to_high.py --hold 0   # 跳过保持 (退化为 v3)
"""
...
def run(client: ArmClient, runner: ArmRunner,
    ...
    """整合 4 阶段: get_yellow → grasp(True) → sleep(hold_s) → high_tower → grasp(False)

    ⚠️ 吸气逻辑 (v5, 2026-07-23 用户第五次明确):
      - 起点: get_yellow 完成 → move_y(-70) 后 grasp(True) 吸气
      - 独立保持: sleep(hold_s) 秒 (默认 5s, 用户指定) — 期间吸盘在 (y=-70, x=-68, hand=DOWN) 持续吸气
    ...
    """
    print(f"\n========== {LOG_PREFIX} run ==========")
    print(f"  吸气窗口: get_yellow 完成 (y=-70) → sleep({hold_s:.1f}s) → high_tower 完成 (x=-160)")
    print(f"  流程: get_yellow → grasp(True) → sleep({hold_s:.1f}s) → high_tower(吸) → grasp(False)")
    print(f"  总吸气时长: {hold_s:.1f}s + high_tower(~4-8s) = ~{hold_s + 4:.1f}-{hold_s + 8:.1f}s")

    # 阶段 1: 取黄位姿 (5 步) —— 吸盘摆到 DOWN 朝下取物位, **吸气前**
    get_yellow_run(client, runner)
    print(f"  >>> get_yellow 完成, y=-70 已就位 <<< 吸气开始触发点")
    ...
    return {
        "ok": True,
        "phases": ["get_yellow", "grasp+sleep", "high_tower", "release", "reset_x"],
        "grasp_window": f"get_yellow_complete(y=-70) → sleep({hold_s:.1f}s) → high_tower_complete(x=-160)",
        "hold_s": hold_s,
        "x_reset": x_reset,
    }
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="task5 test1: get_yellow -> grasp+hold+sleep -> high_tower -> release (吸气包住 high_tower, 5s 独立保持)",
