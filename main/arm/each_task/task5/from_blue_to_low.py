"""task5 / from_blue_to_low —— 「取蓝 → 投低塔」2 阶段串联 (单文件, 非包)。
...
⚠️ **使用原则 (用户要求)**:
  - **不要直接调用** new_get_blue / low_tower 内部的具体动作 (move_y / composite_run / grasp)
  - **调用它们的 run() 函数**: 端到端流程封装, 业务硬限 / composite_run 4 轴全传 /
    grasp 走 runner.* 等全部在两个 run() 内部处理
  - 本文件只负责**串联 + 错误透传**, 不重写任何具体步骤
  - 与 from_blue_to_high.py 同构, 仅 Phase B 不同 (3 步低塔 vs 5 步高塔)

⚠️ **过渡位姿衔接 (隐式, 已在 new_get_blue / low_tower 内处理)**:
  - Phase A 终态: x=0, y=-74 (保护区 [0,-80] 内, 球吸住)
  - Phase B 步骤 1: y=-176 (出保护区 96mm, 低塔投球位姿)
  - 衔接动作: low_tower 步骤 1 move_y 自动把 y 从 -74 抬到 -176
  - arm/hand/x 衔接: Phase A 末态 arm=+85°/hand=+10°/x=0
    → Phase B 步骤 2 composite_run 把 arm→90°/hand=0°/x→-150 (4 轴并发接管)
  - 全程**真空阀不释放**: Phase A grasp(True) 吸住 → Phase B grasp(False) 释放

⚠️ **与 high_tower 的差别**:
  - low_tower 步骤数: 3 (vs high_tower 5)
  - low_tower composite_run x: -150 (vs high_tower -70)
  - low_tower composite_run y: -176 (vs high_tower -185)
  - low_tower composite_run hand: 0° (vs high_tower -82°)
  - low_tower 无 x 推进/回退步骤 (3 步和 5 步的核心差)

⚠️ **业务硬限校验**: new_get_blue 的位姿 (x=0, y=-74/-135, arm=85°, hand=+10°)
...
跑法:
    python main/arm/each_task/task5/from_blue_to_low.py
    python -m main.arm.each_task.task5.from_blue_to_low
"""
...
from main.arm.each_task.task5 import new_get_blue, low_tower  # noqa: E402


# ---------- 流程常量 (仅串联层) ----------

LOG_PREFIX: str = "[task5/from_blue_to_low]"
...
def run(client: ArmClient, runner: ArmRunner) -> dict:
    ...
    print(f"\n========== {LOG_PREFIX} 完成 "
          ...
          f"Phase B y=-176mm 低塔投球, 总耗时 {elapsed:.3f}s) ==========\n")
...
def build_parser() -> argparse.ArgumentParser:
    """CLI 参数: 串联层**不暴露** new_get_blue / low_tower 内部参数。

    原因: 用户明确要求"使用里面的参数和逻辑过程", 即让两个 run() 自己用默认值,
    不在串联层覆写。如果未来需要调参, 直接调 new_get_blue.run / low_tower.run 的
    关键字参数 (这两个 run 都已经把每个步骤的参数做成可调关键字)。
    """
    return argparse.ArgumentParser(
        description=(
            "task5/from_blue_to_low v1: 「取蓝 → 投低塔」2 阶段串联\n"
            "  Phase A: new_get_blue.run (4 机联动 → 吸气 → 下探)\n"
            "  Phase B: low_tower.run    (y 抬升 → 4 机联动 → 释放)\n"
            "  默认耗时 ~6-10s (3+3 步, 比高塔少 2 步 x 推进/回退)"
        ),
