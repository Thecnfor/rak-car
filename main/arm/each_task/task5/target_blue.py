...
目标位姿 (用户指定 2026-07-29):
  ...
  2. **蓝球**识别      (摆位完成后读侧摄 task_feed, 输出蓝球归一化坐标)

最终位姿: x=-40, arm=90°, y=-200, hand=0°。

⚠️ **v2 改: 4 步顺序摆位 → 1 步 composite_run (2026-08-07 用户拍板)**
  - 旧版: move_y(-200) → move_x_with_split(-40) → set_arm_angle(90) → _call_arm(set_hand_angle, 0)
    4 步顺序, 含 belt-slip 安全 move_x (wall_hit / overshoot / FUSE), 耗时 ~5-10s
  - 新版: client.composite_run(arm=90, x_mm=-40, y_mm=-200, hand=0, speed=80, timeout=30)
  ...
  - **去掉 belt-slip 安全 move_x**: composite_run 内部走 SDK `move_x_position` (无 split),
    与 new_get_*.py 一致。前提是 belt-slip 已修复 (2026-07-31 现场验证)
...
40mm 接近 belt-slip 单次有效行程上界 (24-46mm, §7.2.1), 旧版走 _move_x_with_split
分段 + 卡住 kick; v2 沿用 new_get_yellow.py 同款 (-71mm 一次走通), 前提是
belt-slip 已修复 (2026-07-31 现场验证)。"""
...
# v2 新增: composite_run 4 机联动参数 (沿用 new_get_blue.py / new_target.py 同款)
...
def run(client: ArmClient, runner: ArmRunner,
    ...
    """摆到 target1 目标位姿 (composite_run 4 机联动) + 只检测蓝球。
    ...
    print(f"  目标: 4 机联动 composite_run (arm={arm_deg}° x={x_mm}mm y={y_mm}mm hand={hand_deg}°)"
          f"{' → 蓝球识别' if detect else ' (不识别)'}")

    # 1. 4 机联动 composite_run (仿 new_get_blue.py / new_get_yellow.py / new_target.py)
    ...
    # 2. 蓝球识别 (只读, 不动机械臂; 失败只 warn 不抛)
    balls: list = []
    if detect:
        print(f"  [2/2] 蓝球识别 (侧摄 task_feed, ≤{detect_timeout_s}s, "
        ...
        print(f"  [2/2] 蓝球识别  已跳过 (--no-detect)")
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="task5 target_blue v2: 4 机联动 composite_run (arm=90° x=-40 y=-200 hand=0°) + 只检测蓝球",
