"""task5 / from_yellow_to_high —— 「取黄 → 投高塔」2 阶段串联 (单文件, 非包)。
...
跑法:
    python main/arm/each_task/task5/from_yellow_to_high.py
    python -m main.arm.each_task.task5.from_yellow_to_high
"""
...
from main.arm.each_task.task5 import new_get_yellow, high_tower  # noqa: E402


# ---------- 流程常量 (仅串联层) ----------

LOG_PREFIX: str = "[task5/from_yellow_to_high]"
...
def run(client: ArmClient, runner: ArmRunner) -> dict:
    ...
    print(f"\n========== {LOG_PREFIX} 完成 "
          f"(Phase A x=-71 → Phase B x=-70 y=-185 高塔投球, 总耗时 {elapsed:.3f}s) ==========\n")
...
def build_parser() -> argparse.ArgumentParser:
    """CLI 参数: 串联层**不暴露** new_get_yellow / high_tower 内部参数。

    原因: 用户明确要求"使用里面的参数和逻辑过程", 即让两个 run() 自己用默认值,
    不在串联层覆写。如果未来需要调参, 直接调 new_get_yellow.run / high_tower.run 的
    关键字参数 (这两个 run 都已经把每个步骤的参数做成可调关键字)。
    """
    return argparse.ArgumentParser(
        description=(
            "task5/from_yellow_to_high v1: 「取黄 → 投高塔」2 阶段串联\n"
