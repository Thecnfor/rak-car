...
class ArmVisionClient:
    ...
    # ===== 实时追踪（WS 持续 push，不收敛停） =====

    def find_target_track(self, selector: TargetSelector, *,
                          x_mm: float, y_mm: float,
                          hz: float = 30.0,
                          mm_per_norm: float = 30.0,
                          ...
                          max_iter: int = 500,
                          timeout: float = 30.0,
                          on_missing_track: str = "wait",  # 默认不 abort，持续等
                          move_fn: Optional[Callable[[float, float], dict]] = None,
                          ws=None) -> ServoResult:
        """持续实时追踪（永不收敛停）：用 WS subscribe_task_detection 推流，
        每帧算校正后立刻下发 move，循环到 timeout 或 max_iter。

        与 find_target_realtime 的区别：
          - find_target_realtime 收敛（居中）→ stop_event.set → 立即返回 ServoResult(converged=True)
          - find_target_track 不收敛停 —— 持续 push 直到 timeout / max_iter；
            即使目标居中也继续跑（死区 dead-band 抑制 jitter）
          - on_missing_track 默认 "wait"（短暂丢失不 abort），caller 可改 "abort"

        适用场景：目标会移动（边走边跟、流水线抓取、动态避障）。
        不适用：需要"取一次定一位"的场景（用 find_target_realtime）。

        Returns:
            ServoResult —— converged 永远 False（除非 on_missing_track="abort" 且丢了）
        """
        import threading
        ...
        t0 = time.time()
        try:
            # max_iter 兜底 + timeout 兜底 + stop_event（abort 时立即唤醒）
            deadline = t0 + timeout
            while time.time() < deadline and state["iter_count"] < max_iter:
                if stop_event.is_set():
                    break
                # 用短 sleep 让 callback 有机会跑（实际上 callback 在 ws-subscriber 线程）
